...
var HttpClient = module.exports = function HttpClient(host, port, apiKey){
    ...
    this.currentHashrate = function(callback){
        this.get('/infos/current-hashrate', callback);
    }

    this.currentDifficulty = function(callback){
        this.get('/infos/current-difficulty', callback);
    }

    this.chainInfo = function(fromGroup, toGroup, callback){
        this.get(`/blockflow/chain-info?fromGroup=${fromGroup}&toGroup=${toGroup}`, callback);
    }
}
