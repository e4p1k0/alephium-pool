...
var PaymentProcessor = module.exports = function PaymentProcessor(config, logger){
    ...
    const transactionsKey = baseName + ':data:transactions';
    ...
    this.txRewardsKey = function(txId){
        return baseName + ':data:poolrewards:' + txId;
    }

    this.lockRewards = function(txId, changedBalances, callback){
        const redisTx = _this.redisClient.multi();
        const lockedRewardsKey = _this.txRewardsKey(txId);
        redisTx.sadd(transactionsKey, txId);
        const currentMs = Date.now();
        const currentTs = Math.floor(currentMs / 1000);

        for (const address in changedBalances){
            const amount = changedBalances[address];

            // console.log('test 1')
            // _this.redisClient.hincrby(baseName + ':miners:' + address, 'totalPayments', 1, function(error, result) {
            ...
            redisTx.hincrbyfloat(baseName + ':miners:' + address, 'balance', -amount);
            redisTx.hincrbyfloat(baseName + ':miners:' + address, 'paid', amount);
            redisTx.hincrby(baseName + ':miners:' + address, 'totalPayments', 1);
            redisTx.hset(lockedRewardsKey, address, amount);
            redisTx.zadd(baseName + ':' + 'payments:' + address, currentTs, [txId, amount].join(':'));
        }
    ...
    this.onTxConfirmed = function(txId, callback){
        ...
        // redis.zadd(baseName + ':' + 'blocks:confirmed', block.timestamp, [
    ...
    function payment(){
        _this.checkTxConfirmations(function(){
            _this.redisClient.hgetall(balancesKey, function(error, result){
                ...
                // console.log('groupedBalances', groupedBalances, groupedBalances[0].balances)
